...
local function capture_ui()
	local state = require("dotfiles.ui.dap").session_state()
	state.watches = {}
...
local function restore_ui(state)
	...
	require("dotfiles.ui.dap").restore(state)
end
...
-- The debug tab itself is suspended by the lifecycle half of this package.
-- Forward, so every session owner presents ui/session_state.lua one interface.
function M.suspend_for_save()
	require("dotfiles.ui.dap").suspend_for_save()
end

function M.resume_after_save()
	require("dotfiles.ui.dap").resume_after_save()
end
