-- The always-live key layer: every binding that exists from startup, in one place.
--
-- Two kinds deliberately live elsewhere. Keys whose only job is to load a
-- plugin on first press are in keymaps/specs.lua, where lazy.nvim reads them.
-- Buffer-local maps and modal (Hydra) heads belong to the module that owns the
-- runtime object they act on -- an LSP attach, a gitsigns handle, a diff
-- session -- because nothing can bind them before that owner exists.
vim.g.mapleader = " "

local map = vim.keymap.set
local problems = require("dotfiles.lsp.problems")
local project_pickers = require("dotfiles.project.actions.pickers")
local project_info = require("dotfiles.project.info")
local project_reset = require("dotfiles.project.actions.reset")
...
for key, target in pairs({
	...
	map("n", "<leader>y" .. key, function()
		require("dotfiles.core.yank").copy(target[1])
...
local function close_panels()
	require("dotfiles.ui.explorer").close_all()
	pcall(function()
...
if not vim.g.nvim_preview then
	...
	map("n", "<leader>S", function()
		require("dotfiles.lsp.workspace_symbols").open()
	...
	map("n", "<leader>e", function()
		require("dotfiles.ui.explorer").toggle()
...
-- project-rooted. plugins/editor/which-key.lua names the group for which-key; the `Project:`
...
if not vim.g.nvim_preview then
	...
	map("n", "<leader>pl", function()
		require("dotfiles.project.lsp").toggle()
