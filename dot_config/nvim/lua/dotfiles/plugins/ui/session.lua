...
return {
	...
	init = function()
		...
		vim.api.nvim_create_autocmd("VimEnter", {
			once = true,
			callback = function()
				require("dotfiles.project.actions.pruner").prune_stale_sessions({ notify = false })
			end,
	...
	opts = {
		...
		save_extra_data = function()
			return require("dotfiles.ui.session_state").capture()
		end,
		restore_extra_data = function(_, extra_data)
			require("dotfiles.ui.session_state").stage(extra_data)
		end,
		...
		args_allow_files_auto_save = function()
			return require("dotfiles.project.state").is_open()
		end,
		pre_save_cmds = {
			function()
				-- Debug and diff tabs hold runtime-only state. ui/session_state.lua owns
				-- the list of what that covers; this only says when to do it.
				require("dotfiles.ui.session_state").suspend_for_save()
			end,
		},
		post_save_cmds = {
			function()
				require("dotfiles.ui.session_state").resume_after_save()
			end,
		},
		pre_cwd_changed_cmds = {
			function()
				require("dotfiles.project.state").set_open(false)
			end,
		},
		post_restore_cmds = {
			function()
				...
				require("dotfiles.ui.session_state").restore()
			end,
		},
		no_restore_cmds = {
			function()
