return {
	...
	opts = {
		...
		dashboard = {
			...
			preset = {
				-- Dashboard entries, not global bindings: these keys exist only while the
				-- dashboard buffer is on screen.
				keys = {
					{
						icon = " ",
						key = "o",
						desc = "Open Project",
						action = function()
							require("dotfiles.project.actions.pickers").open_directory()
						end,
					},
				...
				explorer = {
					...
					actions = {
						close_explorer = function()
							require("dotfiles.ui.explorer").close()
						end,
					},
					win = {
						list = {
	...
	keys = function()
		return require("dotfiles.keymaps.specs").snacks
	end,
}
