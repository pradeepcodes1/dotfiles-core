...
local function open(lines, all)
	...
	local entries
	if all then
		entries = cli.lines({ "tail", "-n", tostring(lines), path })
	else
		...
		entries = cli.lines({ "grep", "-F", ("[pid:%d]"):format(pid), path })
	...
	vim.bo[buf].modifiable = false
	-- `q` closes this scratch view; buffer-local, so macro recording is untouched.
