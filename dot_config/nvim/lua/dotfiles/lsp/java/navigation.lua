...
local M = {}
local references = require("dotfiles.lsp.references")
...
local function goto_class_by_fqcn(fqcn)
	if not require("dotfiles.project.state").is_open() then
		return
	end
...
function M.on_attach(bufnr)
	...
	-- and only after jdtls has attached to this buffer.
	vim.keymap.set(
		"n",
		"gd",
		smart_definition,
		{ buffer = bufnr, desc = "LSP/Java: [G]oto [D]efinition (import-aware)" }
	)
	vim.keymap.set(
		"n",
		"grr",
		smart_references,
		{ buffer = bufnr, desc = "LSP/Java: [G]oto [R]eferences (import-aware)" }
	)
end
